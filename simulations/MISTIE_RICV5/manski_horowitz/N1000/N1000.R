...
bounds_trial <- boundsNoCov_res(1:6, datasamp$y[datasamp$trt==1], datasamp$y[datasamp$trt==0], maxBen, maxHarm)
...
for (b in 1:B){
  ...
  bounds[b,] <- boundsNoCov_res(1:6, YT, YC, maxBen, maxHarm)
}
...
save(CI, file=paste("N1000","-","seed", val, "-", format(Sys.time(), "%Y%m%d-%H%M"),".Rdata", sep=""))
